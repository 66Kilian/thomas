...
const PAGES: { key: string; label: string; q?: string }[] = [
  { key: 'Home', label: 'Startseite' },
  { key: 'ContentDetail', label: 'Inhalt kaufen', q: '&inhalt=sommer-garten' },
  { key: 'Subscriptions', label: 'Abos' },
  { key: 'Events', label: 'Events' },
  { key: 'Wishlist', label: 'Wunschliste' },
  { key: 'Bundles', label: 'Bundles' },
  { key: 'Auctions', label: 'Auktionen' },
  { key: 'Requests', label: 'Anfragen' },
  { key: 'TextPage', label: 'Über mich', q: '&seite=ueber-mich' },
  { key: 'Wallet', label: 'Konto' },
  { key: 'AgeGate', label: '18+' }
];

const url = (page: (typeof PAGES)[number], w: ThemeKey) => `/v/aurora/${page.key}?s=voll&w=${w}${page.q ?? ''}`;
...
export function DesignWahl() {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[20rem_minmax(0,1fr)] lg:gap-10">
      {/* Designs */}
      <div>
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-white/45">1 · Design wählen</p>
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block text-sm font-bold leading-tight">{THEME_LABELS[k].de}</span>
                  <span className="hidden text-xs text-white/50 sm:block">{STYLE_TEXT[k]}</span>
                </span>
      ...
      <div className="min-w-0">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-white/45">2 · Seite ansehen</p>
        ...
        <div className="mx-auto w-[260px] rounded-[2rem] border border-white/15 bg-[#111] p-2 shadow-2xl shadow-black/60 md:hidden">
          <div className="overflow-hidden rounded-[1.5rem]">
            <Live src={src} width={390} height={844} title={`${page.label} mobil`} />
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-white/15 bg-[#111] shadow-2xl shadow-black/60">
            <div className="flex items-center gap-1.5 border-b border-white/10 px-3 py-2">
              ...
              <span className="ml-3 truncate text-[11px] text-white/40">
                {THEME_LABELS[w].de} · {page.label}
              </span>
            </div>
            <Live src={src} width={1280} height={800} title={`${THEME_LABELS[w].de} · ${page.label}`} />
          </div>
